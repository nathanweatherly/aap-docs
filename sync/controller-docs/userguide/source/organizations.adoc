...
For more information, refer to `xref:ug_users[]`.

==== Add a User or Team

In order to add a user or team to an organization, the user or team must
already be created. See `xref:ug_users_create` and `xref:ug_team_create[]` for
...
particular organization. For more detail, see `xref:ug_notifications_on_off[]`.

...
refer to the `xref:ug_users[]` section for further detail.
...
Access Token (PAT), which is obtained through GitHub. See `xref:ug_webhooks[]`
...
Access Token (PAT), which is obtained through GitLab. See `xref:ug_webhooks[]`
for detail. Entering the provided token is the only required value in
this screen.
...
this concept, refer to `xref:ag_ext_exe_env[]`.
...
`xref:ag_multi_vault[]` section of the Automation Controller Administration
Guide.

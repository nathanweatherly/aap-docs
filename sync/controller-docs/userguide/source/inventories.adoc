...
4.2.1, users can provide the new style inventory plugin config directly
...
. In addition to the update options available for cloud inventory
sources, you can specify whether or not to update on project changes.
Check the *Update on Project Update* option to refresh the inventory
from the selected source after every project update where the SCM
revision changes before executing job tasks. For more detail, refer to
`ag_update_on_project_change` in the Automation Controller

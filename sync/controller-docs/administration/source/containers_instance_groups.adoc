...
as-needed per playbook. For more information, see `xref:ag_container_groups[]`
towards the end of this section.

For execution environments, see `xref:ug_execution_environments[]` in the
...
`Instance Groups \<ug_instance_groups>` in the Automation
...
environment. See `xref:ug_execution_environments[]` in the Automation
...
container group, see `xref:ug_credentials_add[]` in the Automation Controller
...
field. See `xref:ug_inventories_add[]` in the Automation Controller User Guide
for detail.
